...
# Patch Karabo header - needed for Karabo <= 2.11.4
	@python karabo_patch.py
...
src/version.hh: .git/HEAD .git/index .git/refs/tags
	...
	@echo "#ifndef IMAGESOURCE_PACKAGE_VERSION" >> $@
	# Note that --dirty can be fooled: Build once when clean and then change source files - or vice a versa...
	@echo "#define IMAGESOURCE_PACKAGE_VERSION \"$(PACKAGE_NAME)-$(shell git describe --tags --match "*.*.*" --dirty --always )\"" >> $@
	@echo "#endif" >> $@
